    ...
    <Layout>
      ...
      <div className="bg-card border border-border rounded-lg p-5">
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          {CATEGORIES.map((c) => (
            <button key={c} onClick={() => setCats((cs) => cs.includes(c) ? cs.filter((x) => x !== c) : [...cs, c])}
              className={`px-3 py-1 rounded-full text-xs border ${cats.includes(c) ? "bg-[#2E75B6] text-white border-[#2E75B6]" : "border-border"}`}>{c}</button>
          ))}
        </div>
